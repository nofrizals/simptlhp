...
@section('title', 'Obrik Turunan | SIMPTLHP')
@section('page-data', "'basicTables'")

@section('content')
    <div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6">
        <div x-data="{ pageName: `Obrik Turunan` }">
        ...
        <div class="space-y-5 sm:space-y-6">
            <div class="relative border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                ...
                <table id="obrikTurunanTable" class="min-w-full divide-y divide-gray-200 display nowrap">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Id Pegawai</th>
                ...
                <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                    <form id="formObrikTurunan">
                        <div class="-mx-2.5 flex flex-wrap gap-y-5">
                            ...
                            <div class="w-full px-2.5 hidden" id="obrikTurunanRadio">
                                    ...
                                    <div class="flex flex-wrap items-center gap-4">
                                        <div>
                                            <label
                                                ...
                                                <input class="sr-only peer" type="radio" name="obrikTurunanRadio"
                                                    value="0">
                                ...
                                <p class="err text-theme-xs text-error-500" id="obrikTurunanRadio_error"></p>
                            </div>
        ...
        <script>
            $(document).ready(function() {
                ...
                $("#openModalBtn").click(function() {
                    ...
                    $("#modalObrikTurunan").removeClass("hidden opacity-0 pointer-events-none").addClass(
                        "opacity-100 pointer-events-auto");
                });

                $("#closeModalBtn").click(function() {
                    reset();
                    $("#modalObrikTurunan").addClass("opacity-0 pointer-events-none").removeClass(
                        "opacity-100 pointer-events-auto");
                });

                var table = $('#obrikTurunanTable').DataTable({
                    responsive: true,
                    serverSide: true,
                    processing: false,
                    language: {
                    ...
                    ajax: {
                        type: 'POST',
                        url: "{{ url('ajax-data-obrik-turunan') }}",
                    },
                    columns: [{
                            data: 'DT_RowIndex',
                            name: 'DT_RowIndex',
                ...
                $('#formObrikTurunan').submit(function(e) {
                    e.preventDefault();
                    formData = new FormData($('#formObrikTurunan')[0]);
                    $.ajax({
                        type: 'POST',
                        url: "{{ url('obrik-turunan') }}",
                        data: formData,
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        ...
                        success: function(response) {
                            ...
                            if (response.status === false) {
                                ...
                                $('#modalObrikTurunan').addClass(
                                    'hidden opacity-0 pointer-events-none');
                                if ($.fn.DataTable.isDataTable('#obrikTurunanTable')) {
                                    $('#obrikTurunanTable').DataTable().ajax.reload(null, true);
                                }
                                Swal.fire({
                                    title: "Sukses",
                ...
                $('#level').change(function() {
                    ...
                    if (isObrik) {
                        $('#obrikTurunanRadio').removeClass('hidden');
                        $('.pick-tim').addClass('hidden');
                    } else {
                        $('#obrikTurunanRadio').addClass('hidden');
                    ...
                    $('input[name=obrikTurunanRadio][value="0"]').prop('checked', true).trigger('change');
                });

                $(document).on('change', 'input[name=obrikTurunanRadio]', function() {
                    const opd = $('#opd').val();
                    if ($(this).val() != 1) {
                ...
                $(document).on('click', '.btn-deleteObrikTurunan', function() {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                type: "delete",
                                url: "{{ url('obrik_turunan') }}/" + id,
                                dataType: "json",
                                success: function(response) {
                                    if (response.status) {
                                        $('#obrikTurunanTable').DataTable().ajax.reload(
                                            function() {
                                                Swal.fire({
                                                    title: "Sukses",
                                                    text: response.message,
                ...
                $(document).on('click', '.btn-editObrikTurunan', function() {
                    let data = $(this).data();
                    $("#modalObrikTurunan").removeClass("hidden opacity-0 pointer-events-none")
                        .addClass(
                            "opacity-100 pointer-events-auto");
                    ...
                    $('#obrikTurunanRadio').val(data.obrikTurunanRadio);
                ...
                $('.cancel').click(function(e) {
                    e.preventDefault();
                    reset();
                    $('#modalObrikTurunan').addClass('hidden opacity-0 pointer-events-none');
                });

                function reset() {
                    $('#id').val('');
                    $('#formObrikTurunan')[0].reset();
                    $('.err').empty();
                    ...
                    $('input[name="obrikTurunanRadio"][value="0"]').prop('checked', true);
