...
@section('content')
    ...
    {{-- ───── Modal SIMAK ───── --}}
    <div id="modalSimak"
        class="fixed inset-0 z-50 flex items-start justify-center p-5 opacity-0 pointer-events-none transition-opacity duration-300">
        <div class="fixed inset-0 bg-black/10 backdrop-blur-xs"></div>
        <div
                ...
                <div class="rounded-2xl bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                        <div class="-mx-2.5 flex flex-wrap gap-y-5">
                            ...
                            {{-- TOOLBAR --}}
                            <div id="toolbarSimak"
                                class="hidden w-full flex flex-col gap-4 px-6 py-5 md:flex-row md:items-center md:justify-between">
                                {{-- SHOW --}}
                                <div class="flex items-center gap-3">
                                    <span class="text-sm text-gray-500">Tampilkan</span>
                                    <select id="simakPageLength"
                                        class="h-10 rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 outline-none focus:border-brand-500 focus:ring-0 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                                        <option value="10">10</option>
                                        <option value="25">25</option>
                                        <option value="50">50</option>
                                ...
                                {{-- SEARCH --}}
                                <div class="relative">
                                    <input id="simakCustomSearch" type="text" placeholder="Cari pegawai..."
                                        class="h-10 w-72 rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 outline-none focus:border-brand-500 focus:ring-0 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                                </div>
                            </div>

                            {{-- Tabel SIMAK (awalnya tersembunyi) --}}
                            <div class="relative w-full px-6 hidden" id="simakTableWrapper">
                                {{-- Loading --}}
                                <div id="simakTableLoading"
                                    class="hidden absolute inset-0 bg-white/70 dark:bg-gray-900/70 flex items-center justify-center z-50">
                                    <div class="flex flex-col items-center gap-2">
                                        <div
                                ...
                                <table id="simakTable" class="min-w-full text-sm dt-table">
                                    <thead class="bg-gray-50 dark:bg-gray-800">
                                        <tr>
                                            <th
            ...
            <script>
                $(function() {
                    ...
                    function resetSimakForm() {
                        ...
                        $('#toolbarSimak').addClass('hidden');
                        $('#footerSimak').addClass('hidden');
                        $('#simakCustomSearch').off('input.simak');
                        $('#simakPageLength').off('change.simak');
                        $('#simakTable').off('processing.dt.simak');
                        if ($.fn.DataTable.isDataTable('#simakTable')) {
                            $('#simakTable').DataTable().clear().destroy();
                        }
                        $('#simakTable tbody').empty();
                        $('#simakTableInfo').empty();
                        $('#simakTablePagination').empty();
                        $('#simakTableLoading').addClass('hidden');
                    }
                    ...
                    $('#opd_simak').on('change', function() {
                        ...
                        // ==========================================
                        // CLEANUP DATATABLE SEBELUM INIT ULANG
                        // ==========================================
                        ...
                        $('#toolbarSimak').removeClass('hidden');
                        $('#footerSimak').removeClass('hidden');

                        // ==========================================
                        // LOADING
                        // ==========================================

                        $('#simakTable').on('processing.dt.simak', function(e, settings, processing) {
                            $('#simakTableLoading').toggleClass('hidden', !processing);
                        });
                        ...
                        const simakTable = $('#simakTable').DataTable({
                            processing: true,
                            serverSide: false,
                            responsive: false,
                            scrollX: false,
                            ...
                            initComplete: function() {
                                $('#simakTable_info').appendTo('#simakTableInfo');
                                $('#simakTable_paginate').appendTo('#simakTablePagination');
                            }
                        });
                        ...
                        $('#simakCustomSearch')
                            .off('input.simak')
                            .on('input.simak', function() {
                                simakTable.search(this.value).draw();
                            });
                        ...
                        $('#simakPageLength')
                            .off('change.simak')
                            .on('change.simak', function() {
                                simakTable.page.len(this.value).draw();
                            });
                    });
