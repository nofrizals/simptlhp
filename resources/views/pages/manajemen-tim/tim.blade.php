...
@section('title', 'Manajemen Daftar Tim | SIMPTLHP')
@section('page-data', "'daftar-tim'")
                    ...
                    <table id="dataTable" class="min-w-full text-sm">
                        <thead class="bg-gray-50 dark:bg-gray-800">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-gray-500">Nama Tim</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-gray-500">Ketua Tim</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-gray-500">Obrik
                                    Ditangani</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-gray-500">Anggota Aktif
                                </th>
                            ...
                            <div class="w-full px-2.5">
                                ...
                                <select name="ketua" id="ketua"
                                    ...
                                    <option value="" disabled selected
                                        class="text-gray-500 dark:bg-gray-900 dark:text-gray-400">Pilih Ketua</option>
                                    @foreach ($irbans as $irban)
                                        <option value="{{ $irban['id_pegawai'] }}">
                                            {{ ucwords(strtolower($irban['nama_pegawai'])) }}</option>
                                    @endforeach
                                </select>
                                <p class="err text-theme-xs text-error-500" id="ketua_error"></p>
                            </div>
                            <div class="w-full px-2.5">
    ...
    @push('scripts')
        <script>
            $(document).ready(function() {
                ...
                // ─── URL ───────────────────────────────────────────────
                const URL = {
                    ajaxTim: "{{ url('ajax-data-daftar-tim') }}",
                    storeTim: "{{ url('daftar-tim') }}",
                };
                ...
                $('#formTim').submit(function(e) {
                    e.preventDefault();
                    let formData = new FormData($('#formTim')[0]);
                    $.ajax({
                        type: 'POST',
                        url: URL.storeTim,
                        data: formData,
                ...
                $(document).on('click', '.btn-deleteTim', function() {
                    Swal.fire({
                ...
                $(document).on('click', '.btn-editTim', function() {
                    let data = $(this).data();
                    openModal();
                    $('.modal-header').html(
                        'Form Edit Tim');
