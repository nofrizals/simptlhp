...
class StatusTlController extends Controller
{
    ...
    {
        $data = StatusTl::orderBy('id_status', 'desc');
        return DataTables::eloquent($data)
            ...
            ->addColumn('action', function ($value) {
                return '
                    <div class="flex items-center justify-center gap-3 px-4 py-2">
                        <a href="javascript:void(0)" data-id="' . $value->id_status . '" title="Hapus" class="btn-deleteStatusTl text-gray-500 hover:text-red-500 transition duration-200 dark:hover:text-red-400 dark:text-white/90">
                        ...
                        <a href="javascript:void(0)" data-id="' . $value->id_status . '" data-status_tl="' . $value->status_tl . '"  class="btn-editStatusTl text-gray-500 hover:text-blue-600 transition duration-200 dark:hover:text-blue-400 dark:text-white/90" title="Edit">
            ...
            ->filterColumn('status_tl', function (Builder $query, string $keyword) {
                $query->where(function (Builder $q) use ($keyword) {
                    foreach (StatusTl::SEARCHABLE_COLUMNS as $i => $column) {
                        $method = $i === 0 ? 'where' : 'orWhere';
                        $q->{$method}($column, 'LIKE', "%{$keyword}%");
                    }
                });
            })
            ->rawColumns(['status_tl', 'status', 'action'])
            ->make(true);
    }
    ...
    public function store(Request $request)
    {
        $validator  = Validator::make($request->all(), [
            'status_tl' => ['required', 'string', 'max:100']
        ], [
            'status_tl.required'     => 'Status tindak lanjut wajib diisi',
        ]);
        if ($validator->fails()) {
        ...
        $data = [
            'status_tl'      => $validated['status_tl']
        ];

        if ($request->id) {
            $statusTl = StatusTl::find($request->id);
            if (!$statusTl) {
                return response()->json([
                    'status' => false,
        ...
        return response()->json([
            'status'  => (bool) $statusTl,
            'message' => $statusTl ? $message : 'Gagal menyimpan data'
        ]);
    }

    public function destroy(statusTl $statusTl)
    {
        $statusTl->delete();
