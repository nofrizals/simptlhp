...
class NilaiKerugianController extends Controller
{
    ...
    {
        $data = NilaiKerugian::orderBy('id_nilai_kerugian', 'desc');
        return DataTables::eloquent($data)
            ->addIndexColumn()
            ->addColumn('nilai_kerugian', function ($value) {
                $nilaiKerugian = $value->nilai_kerugian;
                return '<span class="dark:text-white/90">' . e($nilaiKerugian) . '</span>';
            })
            ...
            ->addColumn('action', function ($value) {
                return '
                    <div class="flex items-center justify-center gap-3 px-4 py-2">
                        <a href="javascript:void(0)" data-id="' . $value->id_nilai_kerugian . '" title="Hapus" class="btn-deleteNilaiKerugian text-gray-500 hover:text-red-500 transition duration-200 dark:hover:text-red-400 dark:text-white/90">
                        ...
                        <a href="javascript:void(0)" data-id="' . $value->id_nilai_kerugian . '" data-nilai_kerugian="' . $value->nilai_kerugian . '"  class="btn-editNilaiKerugian text-gray-500 hover:text-blue-600 transition duration-200 dark:hover:text-blue-400 dark:text-white/90" title="Edit">
            ...
            ->filterColumn('nilai_kerugian', function (Builder $query, string $keyword) {
                $query->where(function (Builder $q) use ($keyword) {
                    foreach (NilaiKerugian::SEARCHABLE_COLUMNS as $i => $column) {
                        $method = $i === 0 ? 'where' : 'orWhere';
                        $q->{$method}($column, 'LIKE', "%{$keyword}%");
                    }
                });
            })
            ->rawColumns(['nilai_kerugian', 'status', 'action'])
            ->make(true);
    }
    ...
    public function store(Request $request)
    {
        $validator  = Validator::make($request->all(), [
            ...
            'nilai_kerugian.required'     => 'Nilai kerugian wajib diisi',
        ]);
        if ($validator->fails()) {
        ...
        $data = [
            'nilai_kerugian'      => $validated['nilai_kerugian']
        ];

        if ($request->id) {
            $nilaiKerugian = NilaiKerugian::find($request->id);
            if (!$nilaiKerugian) {
                return response()->json([
                    'status' => false,
        ...
        return response()->json([
            'status'  => (bool) $nilaiKerugian,
            'message' => $nilaiKerugian ? $message : 'Gagal menyimpan data'
        ]);
    }

    public function destroy(nilaiKerugian $nilaiKerugian)
    {
        $nilaiKerugian->delete();
