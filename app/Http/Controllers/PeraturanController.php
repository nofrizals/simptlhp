...
class PeraturanController extends Controller
{
    ...
    public function ajaxPeraturan()
    {
        $data = Peraturan::orderByDesc('id_peraturan');
        return DataTables::eloquent($data)
            ->addIndexColumn()
            ->addColumn('judul', function ($value) {
                return '<span class="dark:text-white/90">' . $value->judul ?? '-' . '</span>';
            })
            ->addColumn('keterangan', function ($value) {
                return '<span class="dark:text-white/90">' . $value->keterangan ?? '-' . '</span>';
            })
            ->addColumn('file', function ($value) {
                return '<span class="dark:text-white/90">' . $value->file ?? '-' . '</span>';
            })
            ->addColumn('status', function ($value): string {
                if ($value->edited_by) {
                    return '<span class="dark:text-white/90">Diedit oleh <strong>' . e($value->editedBy->nama_pegawai ?? 'Admin Infokom') . '</strong><br>'
                        . optional(Carbon::parse($value->edited_at ?? '-'))->translatedFormat('d F Y') . '</span>';
                } else {
                    return '<span class="dark:text-white/90">Ditambah oleh <strong>' . e($value->createdBy->nama_pegawai ?? 'Admin Infokom') . '</strong><br>'
            ...
            ->addColumn('action', function ($value): string {
                return '
                    <div class="flex items-center justify-center gap-3 px-4 py-2">
                        <a href="javascript:void(0)" data-id="' . $value->id_peraturan . '" title="Hapus" class="btn-deletePeraturan text-gray-500 hover:text-red-500 transition duration-200 dark:hover:text-red-400 dark:text-white/90">
                        ...
                        <a href="javascript:void(0)" data-id="' . e($value->id_peraturan) . '" data-judul="' . e($value->judul) . '" data-keterangan="' . e($value->keterangan) . '" data-file="' . e($value->file) . '" class="btn-editPeraturan text-gray-500 hover:text-blue-600 transition duration-200 dark:hover:text-blue-400 dark:text-white/90" title="Edit">
            ...
            ->rawColumns(['judul', 'keterangan', 'file', 'status', 'action'])
            ->make(true);
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'file.max'              => 'File yang diupload maksimal 2MB',
        ]);
        ...
        if ($request->id) {
            $peraturan = Peraturan::find($request->id);
            if (!$peraturan) {
                return response()->json([
                    'status'  => false,
            ...
            try {
                if ($request->hasFile('file')) {
                    $newFile = $request->file('file')->store(
                        'peraturan',
                        'public'
                    );
                    if (!$newFile) {
