...
class KasusController extends Controller
{
    ...
    {
        $data = Kasus::orderBy('id_kasus', 'desc');
        return DataTables::eloquent($data)
            ->addIndexColumn()
            ->addColumn('id_jenis_php', function ($value) {
                return $value->id_jenis_php ?? '-';
            })
            ->addColumn('tahun_pemeriksaan', function ($value) {
                return $value->tahun_pemeriksaan ?? '-';
            })
            ->addColumn('spt', function ($value) {
                return $value->spt ?? '-';
            })
            ->addColumn('spt_mulai', function ($value) {
                return $value->spt_mulai ?? '-';
            })
            ->addColumn('spt_selesai', function ($value) {
                return $value->spt_selesai ?? '-';
            })
            ...
            ->addColumn('nip_ketua', function ($value) {
                return $value->nip_ketua ?? '-';
            })
            ->addColumn('action', function ($value) {
                return '
                    <div class="flex items-center justify-center gap-3 px-4 py-2">
                        <a href="javascript:void(0)" data-id="' . $value->id . '" title="Hapus" class="btn-deleteKasus text-gray-500 hover:text-red-500 transition duration-200">
                        ...
                        <a href="javascript:void(0)" data-id="' . $value->id . '"  class="btn-editKasus text-gray-500 hover:text-blue-600 transition duration-200" title="Edit">
            ...
            ->rawColumns(['id_jenis_php', 'tahun_pemeriksaan', 'spt', 'spt_mulai', 'spt_selesai', 'nomor_lhp', 'tanggal_lhp', 'kode_unor', 'nip_ketua', 'action'])
            ->make(true);
    }
    ...
    public function store(Request $request)
    {
        $validator  = Validator::make($request->all(), [
            ...
            'kode_unor.required'            => 'Pilih nama obrik',
            'nip_ketua.required'            => 'Pilih ketua tim'
        ]);
        if ($validator->fails()) {
