...
class KontakController extends Controller
{
    ...
    {
        $data = Kontak::orderByDesc('id_kontak');
        return DataTables::eloquent($data)
            ->addIndexColumn()
            ->addColumn('nama', function ($value) {
                return '<span class="dark:text-white/90">' . $value->nama ?? '-' . '</span>';
            })
            ->addColumn('kontak', function ($value) {
                return '<span class="dark:text-white/90">' . $value->kontak ?? '-' . '</span>';
            })
            ...
            ->addColumn('action', function ($value): string {
                return '
                    <div class="flex items-center justify-center gap-3 px-4 py-2">
                        <a href="javascript:void(0)" data-id="' . $value->id_kontak . '" title="Hapus" class="btn-deleteKontak text-gray-500 hover:text-red-500 transition duration-200 dark:hover:text-red-400 dark:text-white/90">
                        ...
                        <a href="javascript:void(0)" data-id="' . e($value->id_kontak) . '" data-nama="' . e($value->nama) . '" data-kontak="' . e($value->kontak) . '" class="btn-editKontak text-gray-500 hover:text-blue-600 transition duration-200 dark:hover:text-blue-400 dark:text-white/90" title="Edit">
