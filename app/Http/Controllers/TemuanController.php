        ...
        return DataTables::eloquent($data)
            ...
            ->addColumn('total_rekomendasi', function (Temuan $value): string {
                $rekomendasiBadge = '<a href="javascript:void(0)" data-id="' . $value->id_temuan . '" '
                    . 'class="btn-openRekomendasi mt-1 inline-flex items-center gap-1 whitespace-nowrap rounded-full border border-yellow-500 bg-yellow-200 dark:bg-yellow-500 px-3 py-0.5 text-sm font-medium text-yellow-700 dark:text-white/90 transition-all duration-200 hover:border-yellow-300 hover:bg-yellow-100 hover:text-yellow-800">'
            ...
            ->addColumn('temuan', function (Temuan $value): string {
                return '<span class="dark:text-white/90">' . e($value->temuan) ?: '-' . '</span>';
            })
            ->addColumn('penyebab', function (Temuan $value): string {
                return '<span class="dark:text-white/90">' . e($value->penyebab) ?: '-' . '</span>';
            })
            ->addColumn('besaran_kerugian', function (Temuan $value): string {
                return '<span class="dark:text-white/90">Rp ' . number_format((float) $value->besaran_kerugian, 0, ',', '.') . '</span>';
            })
            ->addColumn('besaran_kerugian2', function (Temuan $value): string {
                return '<span class="dark:text-white/90">Rp ' . number_format((float) $value->besaran_kerugian2, 0, ',', '.') . '</span>';
            })
            ->addColumn('besaran_kerugian3', function (Temuan $value): string {
                return '<span class="dark:text-white/90">Rp ' . number_format((float) $value->besaran_kerugian3, 0, ',', '.') . '</span>';
            })
            ->addColumn('besaran_kerugian4', function (Temuan $value): string {
                return '<span class="dark:text-white/90">Rp ' . number_format((float) $value->besaran_kerugian4, 0, ',', '.') . '</span>';
            })
            ...
            ->addColumn('action', function (Temuan $value): string {
                return '
                    <div class="flex items-center justify-center gap-3 px-4 py-2">
                        <a href="javascript:void(0)" data-id="' . $value->id_temuan . '" title="Hapus" class="btn-deleteTemuan text-gray-500 hover:text-red-500 transition duration-200 dark:hover:text-red-400 dark:text-white/90">
                        ...
                        <a href="javascript:void(0)" data-id="' . $value->id_temuan . '"  class="btn-editTemuan text-gray-500 hover:text-blue-600 transition duration-200 dark:hover:text-blue-400 dark:text-white/90" title="Edit">
            ...
            ->rawColumns(['temuan', 'penyebab', 'besaran_kerugian', 'besaran_kerugian2', 'besaran_kerugian3', 'besaran_kerugian4', 'total_rekomendasi', 'log', 'action'])
            ->make(true);
    }
