...
class JenisController extends Controller
{
    ...
    public function ajaxDataJenisPHP()
    {
        ...
        return DataTables::eloquent($data)
            ...
            ->addColumn('jenis_php', function ($value) {
                $jenis = ucwords($value->jenis_php ?? '-');
                return '<span class="dark:text-white/90">' . e($jenis) . '</span>';
            })
            ->addColumn('action', function ($value) {
                return '
                    <div class="flex items-center justify-center gap-3 px-4 py-2">
                        <a href="javascript:void(0)" data-id="' . $value->id_jenis_php . '" title="Hapus" class="btn-deleteJenisPHP text-gray-500 hover:text-red-500 transition duration-200 dark:hover:text-red-400 dark:text-white/90">
                        ...
                        <a href="javascript:void(0)" data-id="' . $value->id_jenis_php . '" data-jenis_php="' . $value->jenis_php . '"  class="btn-editJenisPHP text-gray-500 hover:text-blue-600 transition duration-200 dark:hover:text-blue-400 dark:text-white/90" title="Edit">
            ...
            ->filterColumn('jenis_php', function (Builder $query, string $keyword) {
                $query->where(function (Builder $q) use ($keyword) {
                    foreach (JenisPhp::SEARCHABLE_COLUMNS as $i => $column) {
    ...
    public function destroy(JenisPhp $jenisPhp)
    {
        $jenisPhp->delete();
