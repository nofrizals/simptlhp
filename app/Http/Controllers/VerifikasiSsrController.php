    ...
    public function ajaxDataVerifikasiSsr()
    {
        ...
        return DataTables::eloquent($data)
            ...
            ->addColumn('rincian_keuangan', function ($value) {
                return $value->rincian_keuangan ?? '-';
            })
            ->addColumn('rincian_keuangan2', function ($value) {
                return $value->rincian_keuangan2 ?? '-';
            })
            ->addColumn('rincian_keuangan3', function ($value) {
                return $value->rincian_keuangan3 ?? '-';
            })
            ->addColumn('rincian_keuangan4', function ($value) {
                return $value->rincian_keuangan4 ?? '-';
            })
            ->addColumn('id_status', function ($value) {
                return $value->status->id_status ?? '-';
            })
            ->addColumn('action', function ($value) {
                return '
                    <div class="flex items-center justify-center gap-3 px-4 py-2">
                        <a href="javascript:void(0)" data-id="' . $value->id . '" title="Hapus" class="btn-deleteVerifikasiSsr text-gray-500 hover:text-red-500 transition duration-200">
                        ...
                        <a href="javascript:void(0)" data-id="' . $value->id . '"  class="btn-editVerifikasiSsr text-gray-500 hover:text-blue-600 transition duration-200" title="Edit">
            ...
            ->rawColumns(['tgl_tindak_lanjut', 'tindak_lanjut', 'file', 'rincian_keuangan', 'rincian_keuangan2', 'rincian_keuangan3', 'rincian_keuangan4', 'id_status', 'action'])
            ->make(true);
    }

    // public function edit($id)
    // {
    //     $data = VerifikasiSsr::with('status')->findOrFail($id);
    //     return response()->json([
    //         'id'                => $id,
    //         'tgl_tindak_lanjut' => $data->tgl_tindak_lanjut,
    //         'tindak_lanjut'     => $data->tindak_lanjut,
    //         'file'              => $data->file,
    //         'rincian_keuangan'  => $data->rincian_keuangan,
    //         'rincian_keuangan2' => $data->rincian_keuangan2,
    //         'rincian_keuangan3' => $data->rincian_keuangan3,
    //         'rincian_keuangan4' => $data->rincian_keuangan4,
