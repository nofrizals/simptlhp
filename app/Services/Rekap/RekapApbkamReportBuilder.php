...
final class RekapApbkamReportBuilder
{
    ...
    public function build(string $tahunPemeriksaan, string $kodeUnor): array
    {
        ...
        // 1 query untuk SELURUH kasus di kecamatan ini (bukan per-kampung).
        $allKasus = $this->resolveKasusListForKecamatan($tahunPemeriksaan, $kodeUnor);
        $kasusByKampung = $this->groupKasusByKampung($allKasus, $kampungList, $tahunPemeriksaan);

        // 1 panggilan aggregate() untuk semua kasus (5 query batch total, bukan 5×jumlah kampung).
        $metrics = $this->aggregator->aggregate($allKasus, false);

        $kampungRows = collect();
        $grandTotal = $this->emptyTotals();

        foreach ($kampungList as $index => $kampung) {
            $kasusList = $kasusByKampung[$kampung->kode_turunan] ?? collect();
            $rowsForKampung = collect();
            $kampungTotal = $this->emptyTotals();
    ...
    private function resolveKasusListForKecamatan(string $tahunPemeriksaan, string $kodeUnor): Collection
    {
        if ($tahunPemeriksaan === 'semua') {
            return Kasus::query()
                ->whereNull('deleted_by')
                ->where('id_jenis_php', 2)
                ->where('kode_unor', 'like', $kodeUnor . '%')
                ->orderBy('tahun_pemeriksaan')
                ->get();
        }
    ...
    /**
     * Kelompokkan kasus per kampung, meniru semantik query independen versi lama:
     * - "semua" tahun: kode_unor kasus harus PERSIS SAMA dengan kode_turunan kampung (id_jenis_php=2).
     * - tahun spesifik: kode_unor kasus cukup DIAWALI kode_turunan kampung.
     *
     * @param  Collection<int, Kasus>  $allKasus
     * @param  Collection<int, object{nama_instansi: string, kode_turunan: string}>  $kampungList
     * @return array<string, Collection<int, Kasus>> keyed by kode_turunan
     */
    private function groupKasusByKampung(Collection $allKasus, Collection $kampungList, string $tahunPemeriksaan): array
    {
        $grouped = [];
        foreach ($kampungList as $kampung) {
            $grouped[$kampung->kode_turunan] = collect();
        }

        foreach ($allKasus as $kasus) {
            foreach ($kampungList as $kampung) {
                $matches = $tahunPemeriksaan === 'semua'
                    ? $kasus->kode_unor === $kampung->kode_turunan
                    : str_starts_with((string) $kasus->kode_unor, $kampung->kode_turunan);

                if ($matches) {
                    $grouped[$kampung->kode_turunan]->push($kasus);
                }
            }
        }
    ...
    // statusRatios(), emptyTotals(), addKasusMetrics(), mergeTotals() TIDAK berubah.
}
